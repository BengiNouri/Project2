...
cardata$Color = NULL
cardata$closest_color = NULL
cardata$StandardizedColor = NULL
```
...
cardata$Price <- cardata$Price * 1000
...
cardata <- cardata %>%
  filter(cardata$Price < OutlierUpper)
...
# Remove non-numeric characters like currency symbols, commas, etc.
# Then convert the Price column to numeric.
cardata$Price <- as.numeric(gsub("[^0-9.]", "", cardata$Price))

# Now try plotting again, but let's make sure that the Brand factor levels are not too many for a readable plot
unique_brands <- length(unique(cardata$Brand))
if (unique_brands <= 30) { # If brands are too many, you might not want to plot them directly
  plot(cardata$Price, cardata$Brand)
} else {
  print(paste("Too many unique brands to plot:", unique_brands))
}
...
for (i in 1:n_plots) {
  subset_brands <- brands[((i-1) * n_brands_per_plot + 1):(i * n_brands_per_plot)]
  subset_data <- cardata %>% filter(Brand %in% subset_brands)

  plot_list[[i]] <- ggplot(subset_data, aes(x = Brand, y = Price)) +
    geom_boxplot() +
    theme_minimal() +
    labs(title = paste("Boxplot of Prices for Brands", (i-1) * n_brands_per_plot + 1, "to", min(i * n_brands_per_plot, length(brands))),
         x = "Brand",
         y = "Price")
}
